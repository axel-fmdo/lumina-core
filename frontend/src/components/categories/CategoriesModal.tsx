...
export const CategoryModal = ({ onClose, onSuccess, categoryToEdit }: CategoryModalProps) => {
    ...
    const checkUniqueness = async (field: 'name' , value: string | null | undefined) => {
        ...
        try {
            const response = await api.get('/categories/validate-category-uniqueness', { params: { name: value, category_id: categoryToEdit?.id }});
    ...
    useEffect(() => {
        ...
        // SSolo se resetea si se está montado y abierto
        if (categoryToEdit) {
            reset({
