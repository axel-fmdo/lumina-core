...
def get_dashboard_data(db: Session = Depends(get_db)):
    ...
    # Suma del costo (si es None, poner 0)
    ...
    # 2. Distribución por Categoría (Top 5 con más activos)
    # SQL Equiv: SELECT c.name, COUNT(a.id), SUM(a.cost) FROM assets a JOIN categories c ... GROUP BY c.id
    cat_stats = db.query(
    ...
    # 3. Actividad Reciente (Últimos 7 movimientos)
    recent_activity = db.query(models.AssetHistory)\
        .options(
            ...
            joinedload(models.AssetHistory.asset) # Cargamos el activo también para mostrar el nombre
        )\
